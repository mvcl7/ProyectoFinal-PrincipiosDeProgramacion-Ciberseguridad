...
VERSIONES_SEGURAS = {
    ...
    "dropbear":      {"version": "2022.83", "url": "https://matt.ucc.asn.au/dropbear/releases/"},
    "vnc":           {"version": "6.11.0",  "url": "https://www.realvnc.com/en/connect/download/vnc/"},
    "rdp":           {"version": "10.0",    "url": "https://support.microsoft.com/en-us/windows/remote-desktop"},
    "vsftpd":        {"version": "3.0.5",   "url": "https://security.appspot.com/vsftpd.html"},
    "proftpd":       {"version": "1.3.8",   "url": "http://www.proftpd.org/docs/"},
    "filezilla":     {"version": "1.8.2",   "url": "https://filezilla-project.org/download.php?type=server"},
    "postfix":       {"version": "3.9.0",   "url": "https://www.postfix.org/download.html"},
    "sendmail":      {"version": "8.18.1",  "url": "https://www.proofpoint.com/us/open-source-email-solution"},
    ...
    "mongodb":       {"version": "7.0.11",  "url": "https://www.mongodb.com/try/download/community"},
    "redis":         {"version": "7.2.5",   "url": "https://redis.io/download/"},
    "elasticsearch": {"version": "8.14.1",  "url": "https://www.elastic.co/downloads/elasticsearch"},
    "mssql":         {"version": "16.0",    "url": "https://www.microsoft.com/en-us/sql-server/sql-server-downloads"},
    "bind":          {"version": "9.18.28", "url": "https://www.isc.org/bind/"},
    "unbound":       {"version": "1.20.0",  "url": "https://nlnetlabs.nl/projects/unbound/download/"},
    ...
    "tomcat":        {"version": "10.1.24", "url": "https://tomcat.apache.org/download-10.cgi"},
...
def analizar_versiones(nm, host):
    ancho = 100
    print()
    print("=" * ancho)
    print("  COMPARACION DE VERSIONES")
    print("=" * ancho)
    print(f"{'PUERTO':<12} {'SERVICIO':<15} {'VERSION DETECTADA':<22} {'VERSION MINIMA':<16} ESTADO")
    print("-" * ancho)
    ...
    for proto in nm[host].all_protocols():
        for puerto in nm[host][proto].keys():
            ...
            if not clave:
                sin_datos += 1
                print(f"{puerto_str:<12} {(producto or nombre or 'desconocido'):<15} {version or 'N/A':<22} {'N/A':<16} No registrado")
                continue
            ...
            if not version:
                sin_datos += 1
                print(f"{puerto_str:<12} {clave:<15} {'N/A':<22} {ver_min:<16} Version no detectada")
                continue

            if parsear_version(version) >= parsear_version(ver_min):
                seguros += 1
                print(f"{puerto_str:<12} {clave:<15} {version:<22} {ver_min:<16} SEGURO")
            else:
                desact += 1
                print(f"{puerto_str:<12} {clave:<15} {version:<22} {ver_min:<16} DESACTUALIZADO")
                print(f"   Actualizacion disponible en: {url}")

    print("=" * ancho)
    print(f"  Resumen: {seguros} seguros | {desact} desactualizados | {sin_datos} sin datos")
    print("=" * ancho)
